...
static int
lunpack(lua_State *L) {
    struct toy_socket_message *message = lua_touserdata(L,1);
...
lsendlow(lua_State *L) {
    ...
    int sz = 0;
    void *buffer = get_buffer(L, 2, &sz);
    int err = toy_socket_send_lowpriority(id, buffer, sz);
    lua_pushboolean(L, !err);
    return 1;
}
