...
timer_add(struct timer *T,void *arg,size_t sz,int time) {
	struct timer_node *node = (struct timer_node *)toy_malloc(sizeof(*node)+sz);
...
static struct timer *
timer_create_timer() {
	struct timer *r=(struct timer *)toy_malloc(sizeof(struct timer));
	memset(r,0,sizeof(*r));
...
toy_updatetime(void) {
	...
	if(cp < TI->current_point) {
		fprintf(stderr, "time diff error: change from %lu to %lu", cp, TI->current_point);
		TI->current_point = cp;
