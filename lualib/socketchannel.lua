	...
	while self.__sock do
		...
		if ok and session then
			...
			if co then
				...
				self.__thread[session] = nil
				toy.error("socket: unknown session :", session)
			end
		else
...
local function push_response(self, response, co)
	if self.__response then
		...
		if self.__wait_response then
			toy.wakeup(self.__wait_response)
...
local function connect_backup(self)
	if self.__backup then
		for _, addr in ipairs(self.__backup) do
			...
			toy.error("socket: connect to backup host", host, port)
			local fd = socket.open(host, port, self.__opaque)
			if fd then
				self.__host = host
				self.__port = port
				return fd
...
local function connect_once(self)
	...
	local fd,err = socket.open(self.__host, self.__port, self.__opaque)
	if not fd then
	...
	self.__dispatch_thread = toy.fork(dispatch_function(self), self)
	...
	if self.__auth then
		...
		if not ok then
			...
			if message ~= socket_error then
				...
				toy.error("socket: auth failed", message)
			end
		end
		self.__authcoroutine = false
...
local function try_connect(self , once)
	...
	while not self.__closed do
		...
		if ok then
			if not once then
				toy.error("socket: connect to", self.__host, self.__port)
			end
			return
		elseif once then
			return err
		else
			toy.error("socket: connect", err)
		end
		if t > 1000 then
			toy.error("socket: try to reconnect", self.__host, self.__port)
			toy.sleep(t)
			t = 0
		else
			toy.sleep(t)
...
local function block_connect(self, once)
	...
	if r == nil then
		toy.error(string.format("Connect to %s:%d failed (%s)", self.__host, self.__port, err))
		error(socket_error)
	else
		return r
	end
