...
save_uncomplete(lua_State *L, int fd) {
	struct queue *q = get_queue(L);
	int h = hash_fd(fd);
	struct uncomplete * uc = toy_malloc(sizeof(struct uncomplete));
...
push_more(lua_State *L, int fd, uint8_t *buffer, int size) {
	...
	if (size < pack_size) {
		...
		uc->pack.buffer = toy_malloc(pack_size);
...
filter_data_(lua_State *L, int fd, uint8_t * buffer, int size) {
	...
	if (uc) {
		...
		if (size == pack_size) {
			...
			lua_pushvalue(L, lua_upvalueindex(TYPE_DATA));
			lua_pushinteger(L, fd);
			void * result = toy_malloc(pack_size);
...
static int
lfilter(lua_State *L) {
	struct toy_socket_message *message = lua_touserdata(L,2);
	...
	case TOY_SOCKET_TYPE_CLOSE:
		...
		return 3;
	case TOY_SOCKET_TYPE_ACCEPT:
		lua_pushvalue(L, lua_upvalueindex(TYPE_OPEN));
		// ignore listen id (message->id);
		lua_pushinteger(L, message->ud);
		pushstring(L, buffer, size);
		return 4;
	case TOY_SOCKET_TYPE_ERROR:
		// no more data in fd (message->id)
		close_uncomplete(L, message->id);
...
lpack(lua_State *L) {
	...
	uint8_t * buffer = toy_malloc(len + 2);
...
ltointeger(lua_State *L) {
	void * ptr = lua_touserdata(L, 1);
	lua_pushinteger(L, (int)(intptr_t)(ptr));
	return 1;
}
